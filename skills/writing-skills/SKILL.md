...
| File | Size | Purpose |
|---|---|---|
| `anthropic-best-practices.md` | ~46 KB | Anthropic's official guidance for skill authoring (skill discovery, naming, frontmatter, structure, anti-patterns) |
| `persuasion-principles.md` | ~6 KB | Research foundation for designing high-compliance skills (Cialdini 2021, Meincke et al. 2025 — authority, commitment, scarcity, social proof, unity) |
| `testing-skills-with-subagents.md` | ~13 KB | Complete TDD methodology for skills: pressure scenario design, pressure types (time/sunk cost/authority/exhaustion), plug loopholes, meta-testing |

Load these references lazily — only when needed for the current task.
...
- **Skill destinations**: the original mentions `~/.claude/skills` for Claude Code and `~/.agents/skills/` for Codex. In this catalog, skills are distributed by `install.sh --all` to all supported IDEs (`~/.copilot/skills/`, `~/.devin/skills/`, `~/.config/devin/skills/`, `~/.opencode/skills/`, `~/.config/opencode/skills/`, `~/.cursor/skills/`, `~/.claude/skills/`, `~/.gemini/skills/`, plus base `~/.agents/skills/`).
...
Imported verbatim from [`obra/superpowers/skills/writing-skills`](https://github.com/obra/superpowers/tree/main/skills/writing-skills). All process content (TDD mapping, skill types, structure, anti-patterns, deployment checklist) preserved from upstream. Only the frontmatter and this `Adaptations` / `Origin` section are local additions.
