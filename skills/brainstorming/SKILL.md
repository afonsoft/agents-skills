...
- `visual-companion.md` — full guide for the optional browser-based companion (mockups, diagrams, layout comparisons)
- `spec-document-reviewer-prompt.md` — prompt used by the optional spec self-review pass
- `scripts/` — local Node.js server (`server.cjs`) plus `helper.js`, `frame-template.html` and start/stop shell wrappers that power the Visual Companion. Companion is OPT-IN: text-only brainstorming works without these files.
